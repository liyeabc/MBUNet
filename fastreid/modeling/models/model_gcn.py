...
class AdaptDirGraphGonvLayer(nn.Module):
    ...
    def learn_adj(self, inputs, adj):
        ...
        bs, k, c = inputs.shape
        global_features = inputs[:, k - 1, :].unsqueeze(1).repeat([1, k, 1])
        distances = torch.abs(inputs - global_features)
        distances_gap = []
        position_list = []
        ...
        distances_gap = 15 * torch.cat(distances_gap, dim=1)
        adj_tmp = self.sigmoid(self.scale * self.fc_direct(
            self.bn_direct(distances_gap.transpose(1, 2)).transpose(1, 2))).squeeze()
