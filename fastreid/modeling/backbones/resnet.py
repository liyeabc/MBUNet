...
#from .hub import load_state_dict_from_url as load_url
#
# model_urls = {
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, with_ibn=False, with_se=False):  #这里的block就是上文的Bottleneck类
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(                           #如果输入的通道参数不对，做下采样调整
    ...
    def forward(self, x):
        ...
        for i in range(len(self.layer1)):
            ...
            if i == self.NL_1_idx[NL1_counter]:
                _, C, H, W = x.shape
                x = SELayer(channel=256, reduction=16)
                x = waveblock(x)
                x = SELayer(channel=256, reduction=16)
        ...
        for i in range(len(self.layer2)):
            ...
            if i == self.NL_2_idx[NL2_counter]:
                _, C, H, W = x.shape
                x = SELayer(channel=512, reduction=16)
                x = waveblock(x)
                x = SELayer(channel=512, reduction=16)
...
def build_resnet_backbone(cfg):
   ...
   #
        ...
        #         state_dict = load_url(model_urls[depth])
        #     else:
        #         # ibn resnet
        ...
        # #pretrain_path = 'projects/Black_reid/resnet50-19c8e357.pth'
    #state_dict = torch.load('D:/liujie/HAA-master1 - CELEB-REID/projects/Black_reid/resnet50-19c8e357.pth')
    state_dict = torch.load('/Project0551/guoqing/liujie/CELEB-REID1/projects/Black_reid/resnet50-19c8e357.pth')
