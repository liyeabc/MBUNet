...
def cosine_dist(x, y):
    if isinstance(x, list):
        for i, feature_i in enumerate(x):
            for k, feature_k in enumerate(y):
                bs1, bs2 = feature_i.size(0), feature_k.size(0)
                frac_up = torch.matmul(feature_i, feature_k.transpose(0, 1))
                frac_down = (torch.sqrt(torch.sum(torch.pow(feature_i, 2), 1))).view(bs1, 1).repeat(1, bs2) * \
                            (torch.sqrt(torch.sum(torch.pow(feature_k, 2), 1))).view(1, bs2).repeat(bs1, 1)
