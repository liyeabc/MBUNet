...
import torch
import pdb
from fastreid.modeling.losses.Smooth_AP_loss import SmoothAP
...
def map_losses0(input):
    maploss_dict={}
    loss = SmoothAP(0.01, 32, 4, 2048)
    input = torch.randn(32, 2048, requires_grad=True).cuda()
    loss_map = loss(input)
    maploss_dict['map_loss1'] = loss_map
    return maploss_dict

def map_losses1(input):
    maploss_dict={}
    loss = SmoothAP(0.01, 32, 4, 2048)
    input = torch.randn(32, 2048, requires_grad=True).cuda()
    loss_map = loss(input)
...
def map_losses2(input):
    maploss_dict={}
    loss = SmoothAP(0.01, 32, 4, 2048)
    input = torch.randn(32, 2048, requires_grad=True).cuda()
    loss_map = loss(input)
    maploss_dict['map_loss2'] = loss_map
    return maploss_dict

def map_losses3(input):
    maploss_dict={}
    loss = SmoothAP(0.01, 32, 4, 2048)
    input = torch.randn(32, 2048, requires_grad=True).cuda()
    loss_map = loss(input)
    maploss_dict['map_loss3'] = loss_map
    return maploss_dict

def map_losses4(input):
    maploss_dict={}
    loss = SmoothAP(0.01, 32, 4, 2048)
    input = torch.randn(32, 2048, requires_grad=True).cuda()
    loss_map = loss(input)
    maploss_dict['map_loss4'] = loss_map
    return maploss_dict

def map_losses5(input):
    maploss_dict={}
    loss = SmoothAP(0.01, 32, 4, 2048)
    input = torch.randn(32, 2048, requires_grad=True).cuda()
    loss_map = loss(input)
    maploss_dict['map_loss5'] = loss_map
    return maploss_dict